...
import Tv_Detz from './Tv_Detz'
import { useGetSeriesQuery } from '../app/Tmdb/TmdbSlice'
...
const Tv_index = () => {
    ...
    const {
      ...
      error,
      data: moviesData
    } = useGetSeriesQuery({options: options, type: pathPolifier})
  
  
    let mappedData = [];
  
    if(isSuccess){
      mappedData = moviesData.ids.map((id) => (<Tv_Detz key={id} mdf={moviesData.entities[id]} />))
    }

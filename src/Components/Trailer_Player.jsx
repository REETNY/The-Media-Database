...
const Trailer_Player = (props) => {
    ...
    const {
        ...
        isSuccess,
        error
    } = (type == "anime" && vidUrl.length == 0) ? useAnimeVideosQuery({id}) : ""

    if(type == "anime" && isSuccess){
        const promo = result?.data?.promo;
        const music_vid = result?.data?.music_videos;
        
        const filtered = promo.map(({trailer}, index) => {
            if(!trailer.embed_url)return
            return trailer.embed_url
        })

        vidUrl = [...filtered?.slice(0,4)];
    }
    ...
    return(
        <div style={isOn ? {display: "flex"} : {display: "none"}} className="trailer_video_abso_wrapper">
            <div style={isFullScreen ? {} : {}} className="trailer_wrap_cont">
                ...
                <div className="trailer_wrap_cont_player">
                    {
                      ...
                      ? <iframe key={Math.floor(Math.random() * 1000)} src={(vidUrl != undefined || vidUrl.length != 0) && vidUrl[0]} frameBorder="0"></iframe>
